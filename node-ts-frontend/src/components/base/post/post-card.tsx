...
// Define the IPost interface to ensure type safety
interface IPost {
  _id: string;
  title: string;
  description: string;
  ...
  user: {
    name: string;
  } | null; // The user object can be null
}
...
export default function PostCard({ post }: PostCardProps) {
  ...
  const getInitials = (name: string | null) => {
    if (!name) return '??';
    // CLEANUP: Removed unnecessary curly braces
    return name
      .split(' ')
      ...
      <CardHeader className="pb-3 flex-shrink-0">
        <div className="flex items-center space-x-3">
          <Avatar className="h-8 w-8 flex-shrink-0">
            {/* FIX: Applied optional chaining and a fallback to the 'src' prop */}
            <AvatarImage 
